...
Model.prototype.preSaveOperations = function (inputData, callback) {
  waterfall([
    ...
    next => {
      process.nextTick(() => {
        this.createDocument(inputData, next);
      });
    },
    // Pré validate
    (document, next) => {
...
Model.prototype._savePipe = function (inputData, svFn, callback) {
  waterfall([
    ...
    next => {
      mapSeries(inputData, this.preSaveOperations.bind(this), next);
    },
    // Save document.
    (document, next) => {
    ...
    (document, savedData, next) => {
      mapSeries(document, this.postSaveOperations.bind(this), next);
    }
  ], (err, document) => {
    callback(err, document);
  });
...
Model.prototype.createOne = function (inputData) {
  ...
  return new Promise((resolve, reject) => {
    self.createOnePrivate(inputData, (err, document) => {
      if (err) {
        return reject(err);
      }
      resolve(document);
...
Model.prototype.createMany = function (inputData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self.createManyPrivate(inputData, (err, document) => {
      if (err) {
        return reject(err);
      }
      resolve(document);
