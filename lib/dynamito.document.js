 ...
 *
 * Note: This class can only have functions binded to this.
 * Other atrributes as relative to model class and virtuals.
 */
...
// METHODS
/**
 * Private function to help on save process.
 * This is almost equal creteOne from Commom Document and should become equal.
 * @param inputModel
 * @param callback
 */
function createOne(inputModel, callback) {
  waterfall([
    ...
    next => process.nextTick(() =>
      Core
        .table(inputModel.tableName)
        .put(inputModel, inputModel.schema, (dErr, data) =>
          next(dErr, data))),
    (data, next) => process.nextTick(() => {
      inputModel.update(data);
      inputModel.eventListner.emit(inputModel.tableName, 'save', inputModel);
      inputModel.schema.hook('post', 'save', inputModel, postError => next(postError, data));
    }),
...
Document.prototype.applyVirtuals = function () {
  const virtuals = this.schema.virtualsKeys();
  for (let i = 0; i < virtuals.length; i += 1) {
    const vp = this.schema.virtualpath(virtuals[i]);
...
Document.prototype.extraFields = function () {
  const keys = this.schema.allKeys();
...
Document.prototype.toJSON = function () {
  const keys = this.schema.allKeys();
  const extraKeys = this.extraFields();
  const allKeys = [...keys];
...
Document.prototype.save = function () {
  const self = this;
  return new Promise((resolve, reject) => Core.check(self.tableName, self.schema, (err) => {
    if (err) {
      return reject(err);
    }

    return createOne(self, (createError, data) => {
      if (createError) {
...
Document.prototype.keyAndValue = function () {
  ...
  const skey = this.schema.sortkey();

  const result = {};

  const pstr = this.schema.path(key).meta.stringfy;
  result[key] = pstr(this[key]);

  if (skey !== undefined) {
    const rstr = this.schema.path(skey).meta.stringfy;
