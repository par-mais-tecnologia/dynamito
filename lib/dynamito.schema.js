...
const Path = require('./dynamito.path');
const Hooks = require('./dynamito.hooks');
const DynamitoTypes = require('./schema/index');
...
class Schema {
  ...
  add(obj, prefix) {
    ...
    for (let i = 0; i < keys.length; ++i) {
      ...
      // Create Path
      const createPath = this.path(key, standardObject);
      ...
      if (createPath.keyType !== undefined
        && (createPath.keyType.isPartition || createPath.keyType.isRange)) {
